...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        ...
        logthread(spawn(function()
            local suc,err = pcall(game.HttpGet,game,"https://raw.githubusercontent.com/EnterpriseExperience/ParadiseRPScript/refs/heads/main/new_patch.txt")
...
newButton("Join Discord",function()
    return "Joins The ZEH Discord"
end,
function()
    setclipboard("https://discord.gg/VJh3kkYzBn")
    TextLabel.Text = "Copied invite to your clipboard"
    if request then
        request({Url = 'http://127.0.0.1:6463/rpc?v=1',Method = 'POST',Headers = {['Content-Type'] = 'application/json', Origin = 'https://discord.com'},Body = http:JSONEncode({cmd = 'INVITE_BROWSER',nonce = http:GenerateGUID(false),args = {code = 'VJh3kkYzBn'}})})
    end
end)
